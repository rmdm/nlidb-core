...
NlidbCore.isEmpty = function (arr2d) {
...
NlidbCore.prototype.process = function (tree) {
  if (tree && typeof tree === 'object') {
    var leaf = this.lastLeafInLinearTree(tree);
    if (leaf) { //tree is linear - hence we only need leaf info
      if (Array.isArray(leaf.occurs)) {
    ...
    return [[]]; //proccessed *tree* (pseudo query) should be two dimensions
  }
};
...
NlidbCore.prototype.lastLeafInLinearTree = function lastLeafInLinearTree (tree) {
  if (tree && typeof tree === 'object') {
    if (tree.isLeaf) {
      return tree;
    }
    ...
    if (Array.isArray(subtrees)) {
      if (subtrees.length > 1) {
        ...
        return subtrees[0] ? lastLeafInLinearTree(subtrees[0]) : [];
      }
    } else if (subtrees){
      return lastLeafInLinearTree(subtrees);
    } else {
      return [];
    }
  } else {
